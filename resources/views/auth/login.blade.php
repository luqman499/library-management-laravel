...
<body class="h-screen flex items-center justify-center px-6 overflow-hidden">
    ...
    <div class="w-full max-w-sm">
        <div class="bg-white/[0.06] backdrop-blur-xl border border-white/10 rounded-2xl px-7 py-6">

            <!-- Icon + Title -->
            <div class="text-center mb-5">
                <div class="w-10 h-10 bg-indigo-600 rounded-lg flex items-center justify-center mx-auto mb-3 glow">
                    <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.746 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                    </svg>
                </div>
                <h1 class="text-lg font-bold text-white mb-0.5">Welcome back</h1>
            ...
            @if (session('status'))
                <div
                    class="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs px-3 py-2 rounded-lg mb-4">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('login') }}">
                ...
                <div class="mb-3">
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email') }}" required autofocus
                        ...
                        class="w-full bg-white/[0.06] border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition">
                    @error('email')
            ...
            <!-- Back to home -->
            <p class="text-center mt-3 pt-3 border-t border-white/5">
