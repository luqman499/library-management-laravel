...
<body class="bg-slate-50 min-h-screen">
    ...
    <div class="max-w-6xl mx-auto px-6 py-10">
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="{{ route('borrow.create') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition">+ New Borrow</a>
        </div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50 transition">
                        <td class="px-5 py-3 text-slate-400">{{ $borrow->id }}</td>
                        ...
                        <td class="px-5 py-3">
                            @if ($borrow->return_date)
                                <span class="bg-emerald-50 text-emerald-600 text-xs font-medium px-2 py-1 rounded-full">Returned</span>
                            @else
                                <span class="bg-amber-50 text-amber-600 text-xs font-medium px-2 py-1 rounded-full">Borrowed</span>
                            @endif
                        </td>
                        <td class="px-5 py-3">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('borrow.show', $borrow->id) }}" class="text-xs bg-slate-100 hover:bg-slate-200 text-slate-700 px-3 py-1.5 rounded-md transition">View</a>
                                <a href="{{ route('borrow.edit', $borrow->id) }}" class="text-xs bg-amber-50 hover:bg-amber-100 text-amber-700 px-3 py-1.5 rounded-md transition">Edit</a>
                                <form method="POST" action="{{ route('borrow.destroy', $borrow->id) }}" onsubmit="return false;">
                                    @csrf
                                    @method('DELETE')
