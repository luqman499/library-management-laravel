...
<body class="bg-slate-50 min-h-screen">
    ...
    <nav class="bg-white border-b border-slate-200 px-8 py-4 flex items-center justify-between">
        ...
        <div class="flex gap-6 text-sm font-medium text-slate-500">
            ...
            <a href="{{ route('member.index') }}" class="text-indigo-600 border-b-2 border-indigo-600 pb-0.5">Members</a>
    ...
    <div class="max-w-6xl mx-auto px-6 py-10">
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="{{ route('member.create') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition">+ Add Member</a>
        </div>

        @if (session()->has('success'))
            <div id="successAlert" class="flex items-center justify-between bg-emerald-50 border border-emerald-200 text-emerald-700 text-sm px-4 py-3 rounded-lg mb-6">
                <span>✅ {{ session('success') }}</span>
                <button onclick="this.parentElement.remove()" class="text-emerald-500 hover:text-emerald-700 ml-4 font-bold">✕</button>
            </div>
        @endif
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50 transition">
                        <td class="px-5 py-3 text-slate-400">{{ $member->id }}</td>
                        <td class="px-5 py-3">
                            @if ($member->photo)
                                <img src="{{ asset('uploads/members/' . $member->photo) }}" alt="{{ $member->name }}" class="w-10 h-10 object-cover rounded-full shadow-sm">
                            @else
                                <div class="w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center text-slate-400 font-semibold text-sm">
                        ...
                        <td class="px-5 py-3">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('member.show', ['member' => $member]) }}" class="text-xs bg-slate-100 hover:bg-slate-200 text-slate-700 px-3 py-1.5 rounded-md transition">View</a>
                                <a href="{{ route('member.edit', ['member' => $member]) }}" class="text-xs bg-amber-50 hover:bg-amber-100 text-amber-700 px-3 py-1.5 rounded-md transition">Edit</a>
                                <form method="POST" action="{{ route('member.destroy', ['member' => $member]) }}" onsubmit="return false;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" onclick="confirmDeleteModal(this.closest('form'))" class="text-xs bg-red-50 hover:bg-red-100 text-red-600 px-3 py-1.5 rounded-md transition">Delete</button>
                                </form>
                            </div>
                        </td>
                    </tr>
