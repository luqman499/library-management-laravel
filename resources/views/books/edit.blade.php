...
<body class="bg-slate-50 min-h-screen">
    ...
    <div class="max-w-2xl mx-auto px-6 py-10">
        <div class="mb-6">
            <a href="{{ route('book.index') }}" class="text-sm text-slate-400 hover:text-slate-600 transition">← Back to Books</a>
                ...
                <div class="mb-6">
                    ...
                    @if ($book->image)
                        <img src="{{ asset('uploads/books/' . $book->image) }}" alt="{{ $book->title }}"
                             class="w-16 h-24 object-cover rounded-lg shadow-sm mb-3">
                    @endif
                    ...
                    <p class="text-xs text-slate-400 mt-1">Leave empty to keep current image. Allowed: JPG, JPEG, PNG, WEBP</p>
                    @error('image') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div class="flex gap-3">
                    <button type="submit" class="bg-amber-500 hover:bg-amber-600 text-white text-sm font-medium px-5 py-2.5 rounded-lg transition">Update Book</button>
                    <a href="{{ route('book.index') }}" class="bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-medium px-5 py-2.5 rounded-lg transition">Cancel</a>
                </div>
